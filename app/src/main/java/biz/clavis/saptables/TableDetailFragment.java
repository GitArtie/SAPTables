...
public class TableDetailFragment extends Fragment {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (getArguments().containsKey(ARG_ITEM_ID)) {
            ...
            JSONSingleton jsonsing = JSONSingleton.getInstance();
            mItem = jsonsing.ITEM_MAP.get(getArguments().getString(ARG_ITEM_ID));

            Activity activity = this.getActivity();
            CollapsingToolbarLayout appBarLayout = (CollapsingToolbarLayout) activity.findViewById(R.id.toolbar_layout);
            if (appBarLayout != null) {
                appBarLayout.setTitle(mItem.table);
            }
        }
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if (mItem != null) {
            ((TextView) rootView.findViewById(R.id.table_detail)).setText(mItem.description + "\n" + mItem.group);
        }
